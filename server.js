...
const qaBank = [
  ...
  { keys: ["vitamin","a","good","for"], answer: "Vitamin A is important for good vision, immune function, and healthy skin." },
  { keys: ["vitamin","b12","good","for"], answer: "Vitamin B12 helps make red blood cells and keeps nerves healthy. Found mainly in animal foods." },
  { keys: ["calcium","good","for"], answer: "Calcium builds strong bones and teeth and helps muscles and nerves work properly." },
  { keys: ["iron","good","for"], answer: "Iron helps make red blood cells that carry oxygen throughout the body." },
  { keys: ["magnesium","good","for"], answer: "Magnesium supports muscle function, nerve function, and helps regulate blood sugar and blood pressure." },
...
function findStaticAnswer(question) {
  ...
  for (const entry of qaBank) {
    ...
    for (const keyword of entry.keys) {
      if (q.includes(keyword)) score += 2;          // full keyword match
      else if (words.some(w => keyword.includes(w) && w.length > 3)) score += 1; // partial
    }
    if (score > bestScore) {
      bestScore = score;
...
async function getGroqAnswer(question) {
  const completion = await groq.chat.completions.create({
    ...
    messages: [
      {
        role: "system",
        content: "You are a simple health facts assistant for the general public. Answer ONLY in 1 to 2 short simple lines. Use words a school student can understand. No medical jargon. No warnings. No disclaimers. Just clean simple facts.",
      },
      {
        role: "user",
        content: "Answer this health question in 1-2 simple lines: " + question,
      },
    ],
  });
...
app.post("/api/ask", async (req, res) => {
  ...
  // Layer 1: Smart static match
  const staticAnswer = findStaticAnswer(question);
  if (staticAnswer)
    return res.json({ success: true, question, answer: staticAnswer, source: "static" });

  // Layer 2: Groq AI for anything else
  try {
    const aiAnswer = await getGroqAnswer(question);
    return res.json({ success: true, question, answer: aiAnswer, source: "Wikipedia" });
  } catch (err) {
...
app.listen(3000, () => {
  ...
  console.log("Health Q&A ready with " + qaBank.length + " topics + Groq AI fallback!");
});